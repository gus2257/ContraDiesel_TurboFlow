(function(window, $, undefined) {
    ...
    Grid.prototype = {
        ...
        _createBody: function() {
            ...
            var $bodyGrid = this._bodyGrid = $("<table id=" + this.id + ">").addClass(this.tableClass)
                .append($content);
        ...
        _refreshContent: function() {
            ...
            for(var itemIndex = indexFrom; itemIndex < indexTo; itemIndex++) {
                ...
                $content.append(this._createRow(item, itemIndex, rowIndex));
                rowIndex = rowIndex + 1;
            }
        },
        ...
        _createRow: function(item, itemIndex, rowIndex) {
            var $result;

            if($.isFunction(this.rowRenderer)) {
                $result = $(this.rowRenderer(item, itemIndex, rowIndex));
            } else {
                $result = $("<tr>");
        ...
        _refreshHeight: function() {
            ...
            if(height !== "auto") {
                ...
                //Cuando IE < 11.0 entonces tiene problemas al sacar el height
                if($.browser.msie)
                {
                    if (parseFloat($.browser.version) < 11 && height <= 100) {
                        height = GetHeightFormContent() - container.context.offsetTop;  

                        if (container.context.offsetTop == 0) {
                            height = height - 100;
                        }
                    }
                } 
                ...
                if ((height - nonBodyHeight) > 170) {
                    this._body.outerHeight(height - nonBodyHeight);
                }
                else {
                    //this._body.outerHeight(170);
                }
            }
        },
