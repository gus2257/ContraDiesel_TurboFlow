...
var Ex = {
    ...
    DateFromUTC: function (value, formatString) {

        intValue = parseInt(value.replaceAll("/", "").replace("Date(", "").replace(")", ""));

        dateValue = new Date(Date.UTC((new Date(intValue)).getFullYear(), (new Date(intValue)).getMonth(), (new Date(intValue)).getDate(), (new Date(intValue)).getHours(), (new Date(intValue)).getMinutes(), (new Date(intValue)).getSeconds()));
    ...
    mensajes: function (Amsj, tipomsj, Atitle, AokText, AcancelText, AObj_OkConfirm, AObj_CancelConfirm, parametro) {
        try {
            ...
            if (Atitle == null && tipomsj == 4) Atitle = Ex.GetGlobalResourceValue("Confirmacion") == '' ? 'Confirmación' : Ex.GetGlobalResourceValue("Confirmacion");
            if (Atitle == null && tipomsj == 5) Atitle = Ex.GetGlobalResourceValue("Aviso") == '' ? 'Aviso' : Ex.GetGlobalResourceValue("Aviso");
    ...
    ExistsPropertyInArray: function (arreglo, valor, key) {
        for (var i = 0; i < arreglo.length; i++) {
            var value = arreglo[i][key];
            if (value == valor) {
                return true;
            }
        }
    ...
    GroupArray: function group(list, prop, valid,name) {
        return list.reduce(function (grouped, item) {
            var key = Ex.IsFunction(prop) ? prop.apply(this, [item]) : item[prop];
            // Si Valida que el item sea mayor a 0
            if (valid) {
                if (item[prop] > 0) {
    ...
    GroupArrayToArray: function group(arrayDe, prop, arrayA) {
        var groupRet;
        groupRet = arrayDe.reduce(function (grouped, item) {
            var key = Ex.IsFunction(prop) ? prop.apply(this, [item]) : item[prop];
            grouped[key] = grouped[key] || [];
            grouped[key].push(item);
            var overWrite = arrayA.filter(function (a) { return a.embalajeId == key })[0];
            overWrite.merge = grouped[key]
            return grouped;
        },
        {});
        return arrayA;
