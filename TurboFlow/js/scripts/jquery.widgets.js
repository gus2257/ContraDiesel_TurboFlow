        ...
        _create: function () {
            ...
            self.element.autocomplete({
                source: function (request, response) {
                    ...
                    datosactual[self.options.PropertyKeyName] = request.term;
                ...
                select: function (event, ui) {
                    ...
                    //JVillarreal .- Se comentarizan las siguientes lineas ya que provocaban doble consulta.
                ...
                if (self.options.fwidth != undefined) {
                    styleAdd='style=margin-left:-' + self.options.fwidth + 'px;';
                }

                var copyItems = [];
                if (pageCount > 15) {
                    $(" <li id='header' class='{0}' " + styleAdd + ">{1}</li>".format(self.options.MessageCss, self.options.TooManyResultsMessage)).appendTo(ul);
                ...
                self.TotalWidth = 0;
                    ...
                    var header = "<li class='{0}' " + styleAdd + ">".format(self.options.HeaderCss);
                ...
                if (self.options.ShowHeader) {
                    $(header).width(self.TotalWidth   + 30).appendTo(ul);
                }
...
//            self.element.removeClass("ui-corner-all");
        ...
        _RenderItem: function (ul, item) {
            ...
            $.each(self.options.PropertiesToShow, function (index, property) {
                elements = elements + "<span style='width:{0}px; float:left;' >{1}</span>".format(property.Width, item[property.PropertyName]);
            });

            if (self.options.fwidth != undefined)
            {
                styleAdd='margin-left:-' + self.options.fwidth + 'px;';
            }

            return $("<li style='padding-top: 2px; padding-bottom: 2px; " + styleAdd + "' ></li>")                            
